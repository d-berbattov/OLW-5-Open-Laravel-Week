...
{
  public function buildMenu() // load menu
  {
    $brands = Brand::all();
  ...
  public function productAssessories() {         //Load filter content
  ...
  public function products(Request $request) {  //Product list
    ...
    $products = Cache::tags(['product_related'])->rememberForever($key, function() use ($request) {
      ...
      $products = $products->paginate(12); //Pagination
  ...
  public function product(Product $product) { // Return products
